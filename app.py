...
if "interface" not in st.session_state :
    st.session_state.interface = False
...
if "import_output" not in st.session_state :
    st.session_state.import_output = False
...
if "azure_endpoint" not in st.session_state :
    st.session_state.azure_endpoint = None  
...
def callback():
    st.session_state.import_output = True
...
def excel_download1(df):
    ...
    csv_file = df.to_csv(index=False)
    b64 = base64.b64encode(csv_file.encode()).decode()  # Encodes the CSV file as base64
    href = f'<a href="data:file/csv;base64,{b64}" download="data.csv">Download Jira Import CSV</a>'  # Creates download link
...
def main():
                    ...
                    with st.spinner('Loading...'):
                        ...
                        # Creating and storing threads for each row of chunk df in thread_list
                            for index, row in sub_df.iterrows():
                                ...
                                threads_list.append(Thread(target=call_crew, args=(single_row_df,output_df_list,st.session_state.credentials,)))
                        ...
                        #creating df to be downloaded by the user, which can be imported directly by the user.
                        import_jira_data = output[["Ticket Key", "Summary Original", "New Description", "Expected Story Points", "Ticket Type", "Description Original", "Story Points Original"]]
                        import_jira_data['New Description'] = import_jira_data['New Description'].fillna(import_jira_data['Description Original'])
                        import_jira_data['Expected Story Points'] = import_jira_data['Expected Story Points'].fillna(import_jira_data['Story Points Original'])
                        column_rename_mapping = {
                        'Ticket Key': 'Issue Id', 
                        'Summary Original': 'Summary', 
                        'New Description': 'Description', 
                        'Expected Story Points': 'Story point estimate',
                        'Ticket Type': 'Issue Type'
                        }

                        import_jira_data.rename(columns=column_rename_mapping, inplace=True)
                        import_jira_data = import_jira_data.drop(["Description Original","Story Points Original"],axis=1)
                        ...
                        with col2:
                            # st.download_button("Jira-Import Output",excel_download1(st.session_state.import_jira_data),"Jira-Import Output.csv",'text/csv')
                            st.markdown(excel_download1(import_jira_data), unsafe_allow_html=True)
        ...
        if project_name:    
            ...
            if st.button("Execute Crew"):
                with st.spinner('Loading...'):
                    ...
                    df_new_api = jira_dataframe.head(1)
                    mini_df_list = []
                    chunk_size = 5
                    ...
                    for sub_df in mini_df_list:
                        ...
                        for index, row in sub_df.iterrows():
                            single_row_df = pd.DataFrame([row])
                            thread_1 = Thread(target=call_crew_api, args=(single_row_df,output_df_list_api,st.session_state.credentials,))
                    ...
                    for index,row in output1.iterrows():
                        ...
                        import_jira_data1 = output1[["Ticket Id", "Ticket Key", "Summary Original", "New Description", "Expected Story Points", "Ticket Type", "Description Original", "Story Points Original"]]
                        import_jira_data1['New Description'] = import_jira_data1['New Description'].fillna(import_jira_data1['Description Original'])
                        import_jira_data1['Expected Story Points'] = import_jira_data1['Expected Story Points'].fillna(import_jira_data1['Story Points Original'])
                        column_rename_mapping = {
                        'Ticket Id': 'Issue Id', 
                        ...
                        import_jira_data1.rename(columns=column_rename_mapping, inplace=True)
                        import_jira_data1 = import_jira_data1.drop(["Description Original","Story Points Original"],axis=1)
    ...
    if st.session_state.default:
        ...
        st.title("Please enter your OpenAI credentials")

        
        column1, column2 = st.tabs(["OpenAI", "Azure OpenAI"])
        with column1:
            
            st.subheader("Enter your OpenAI Key")
            api_key = st.text_input("Enter your OpenAI Key")
            submit_key1 = st.button("Submit")
            if submit_key1:
                print(st.session_state.interface)
                st.session_state.api_key = api_key
                st.session_state.credentials = {
                                                "api_key": st.session_state.api_key,
                                                "api_endpoint": st.session_state.api_endpoint
                                                }
                st.session_state.interface = True
                print("Credentials are: ",st.session_state.credentials)
                print("API Key is: ",st.session_state.api_key)

        with column2:
            
            st.subheader("Enter your Azure OpenAI Key")
            st.session_state.api_key = st.text_input("Enter your Azure OpenAI Key") 
            st.session_state.api_endpoint = st.text_input("Enter your Azure Endpoint")
            submit_key2 = st.button("Submit Key")
            if submit_key2:
                st.session_state.credentials = {
                                                "api_key": st.session_state.api_key,
                                                "api_endpoint": st.session_state.api_endpoint
