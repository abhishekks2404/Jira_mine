...
def crew_agent(ticket_detail,credentials): 
   
    if credentials["api_endpoint"] == '':
        print("Executed OpenAI")
        print("Api Key is: ",credentials["api_key"])
        llm = ChatOpenAI(model="gpt-4-1106-preview",api_key=credentials["api_key"])
    
    else:
        print("Executed AZURE OpenAI")
    ...
    print({"handling_gpt":output})
    return output[0]["output"]
     

def generate_url(ticket_key):
    print(ticket_key)
    base_url = 'https://blenheimchalcot.atlassian.net/browse/'
    return base_url+str(ticket_key)

# def make_clickable(url, name):
#     return '<a href="{}" rel="noopener noreferrer" target="_blank">{}</a>'.format(url,name)
               
def append_row(dataframe,values,ticket_id,ticket_key,ticket_summary,ticket_description,ticket_story_points,ticket_type,labels,components,epic_link):
    values_dict = values
    print(ticket_description)
    data_to_append = {  'Ticket Id': str(ticket_id),
    ...
    for index, row in df.iterrows():
        try:
            row_df = pd.DataFrame([row])
             #creating a dataframe of single row so that each row can be given as input to GPT for futher processing.
             # Loop through the list of columns and add the column if it doesn't exist
            columns_to_add = ['Labels', 'Components', 'Epic Link Summary']
            for column in columns_to_add:
                if column not in row_df.columns:
                    row_df[column] = None
            print("------Hi-----",row_df)
            mini_df = row_df[["Issue id","Issue key","Summary","Description","Custom field (Story Points)","Labels","Components","Epic Link Summary"]]  #only those columns which are important.
            ticket_id = row_df["Issue id"][index]  
            ticket_key = row_df["Issue key"][index]    #taking values of each columns as it is to store it later in the output for each row one by one.
            ...
            result = crew_agent(mini_df,credentials)
        # st.write(result)
            output_df = append_row(output_df,result,ticket_id,ticket_key,ticket_summary,ticket_description,ticket_story_points,ticket_type,labels,components,epic_link)
            output_df['ticket_url'] = output_df['Ticket Key'].apply(generate_url)
        ...
        # output_df['Ticket link'] = output_df.apply(lambda x: make_clickable(x['url'], x['Ticket Key']), axis=1)
        # output_df['Ticket link'] = output_df.apply(lambda x: make_clickable(x['url'], x['Ticket Key']), axis=1)
